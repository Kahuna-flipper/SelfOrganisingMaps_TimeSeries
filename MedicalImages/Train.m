...
for i=1:iterations
    ...
    for j=1:p
     ...
     [~,index]=min(dist);   
     l = ceil(index/column);
     q = index - column*(l-1); % l and q store index of neuron for weight adjustment
    ...
    if(sigma>0)     
        for r=1:sigma
             ...
             if(l<row && q+r<column)
                 ...
                 weights(temp_index,:) = weights(temp_index,:) + alpha*exp(-(dtw(weights(temp_index,:),weights(index,:)))/(2*r^2))*(sample_vector-weights(temp_index,:));
             end
             if(l<row && q-r>0)
